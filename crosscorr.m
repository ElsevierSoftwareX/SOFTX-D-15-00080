...
%   $Revision: 1.1.8.3 $  $Date: 2009/10/10 20:05:12 $

if nargin < 2
    
   error('econ:crosscorr:UnspecifiedInput',...
         'Two series are required.')
...
if ((rows ~= 1) && (columns ~= 1)) || (rows*columns < 2)
   ...
   error('econ:crosscorr:NonVectorSeries1',...
...
if ((rows ~= 1) && (columns ~= 1)) || (rows*columns < 2)
   ...
   error('econ:crosscorr:NonVectorSeries2',...
         'Second series must be a vector.')
...
if (nargin >= 3) && ~isempty(numLags)
   ...
   if numel(numLags) > 1
      ...
      error('econ:crosscorr:NonScalarLags',...
            'Number of lags must be a scalar.')
        
   end
   
   if (round(numLags) ~= numLags) || (numLags <= 0)
       
      error('econ:crosscorr:NonPositiveInteger',...
            'Number of lags must be a positive integer.')
        
   end
   
   if numLags > (N-1)
       
      error('econ:crosscorr:InputTooLarge',...
            'Number of lags must not exceed \nthe minimum series length minus one.')
...
if (nargin >= 4) && ~isempty(numSTD)
   ...
   if numel(numSTD) > 1
      ...
      error('econ:crosscorr:NonScalarSTDs',...
   ...
   if numSTD < 0
      ...
      error('econ:crosscorr:NegativeSTDs',...
